...
Eight self-contained demos of what weave can do — **all offline, no `ANTHROPIC_API_KEY`**. Peers use
the `--fake` echo worker or deterministic code skills; every demo runs in a throwaway workspace under
`/tmp/weave-demos/`, never the engine repo.
...
Each demo is a small bash script in this directory (plus `federated.mts` for #6 and the code skills in
`skills/`); read them as worked examples. `lib.sh` holds the shared helpers.
