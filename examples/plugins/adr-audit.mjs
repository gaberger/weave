...
export default {
  name: "adr-audit",
  description: "Deterministically reconcile ADR statuses (file ⟷ INDEX) and advance finished ones (no LLM).",
  ...
  async run(task, ctx) {
    ...
    const summary = parts.length ? parts.join(" | ") : "all ADRs already reconciled (no-op)";

    return {
      status: "completed",
      summary,
      artifacts: [{ kind: "adr-audit", ref: JSON.stringify({ advanced, reconciled, conflicts }) }],
    };
  },
};
