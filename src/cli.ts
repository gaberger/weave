...
async function cmdChat(args: Args): Promise<void> {
  ...
  for (;;) {
    ...
    // The pin is SOFT for a conversational turn (see softPin): the daemon's router predicate-routes,
    // so a domain turn (CVE/vuln → forward-vulnerability, research → research) reaches its specialized
    // skill while general chat falls through to the persona catch-all. No client-side keyword yield.
    turnAbort = new AbortController();
