...
/** Fold the log: compute settled subjects, the active subject set, and the high-water seq.
 *  Pure, domain-agnostic (ADR-0007). */
export function compact(events: readonly SealedEvent[]): Compaction {
  ...
  for (const e of events) {
    ...
    if (e.kind === TaskKind.Declared) declared.add(e.subject);
  }

  const activeSubjects = new Set<string>();
  for (const s of declared) if (!settled.has(s)) activeSubjects.add(s);

  return { payload: { upTo, settled: [...settled] }, activeSubjects };
}
